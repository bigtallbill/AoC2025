defmodule Aoc2025.D4P2 do
  ...
  def reduce_grid(:halt, total_removed), do: total_removed

  def reduce_grid({w, h, map} = grid, total_removed) do
    neighboring = grid |> calculate_neighbors() |> Enum.map(&count_neighboring_rolls?(&1))

    can_remove =
      neighboring |> Enum.filter(&(&1 |> elem(4) < 4 and &1 |> elem(2) == "@"))

    cond do
      length(can_remove) == 0 ->
        reduce_grid(:halt, total_removed)

      true ->
        map =
          Enum.reduce(can_remove, map, fn {x, y, _char, _neighbors, _count}, map ->
            update_at(map, {x, y}, ".")
          end)

        total_removed = total_removed + length(can_remove)

        reduce_grid({w, h, map}, total_removed)
    end
  end
  ...
  def calculate_neighbors({w, h, map}) do
  ...
  def update_at(map, {x, y}, new_content) do
    row = map |> Map.get(y, %{})

    new_row =
      row
      |> Map.update(x, new_content, fn existing_value ->
        new_content
      end)
  ...
  def count_neighboring_rolls?({x, y, char, neighbors}) do
    count =
      neighbors
